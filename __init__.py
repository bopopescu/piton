...
def input():
        ...
        query = "insert into padi(id, luas_lahan, tgl_tanam, tgl_siap_panen, hasil_panen, pemilik, nik, pekerja) VALUES( %s, %s, %s, %s, %s, %s, %s, %s)"
        eksekusi = cur.execute (query,(id, luas_lahan, tgl_tanam, tgl_siap_panen, hasil_panen, pemilik, nik, pekerja))
...
@app.route('/padi/update/<int:id>', methods = ['PUT'])
def update(id):
        cur = mysql.connection.cursor()
        luas_lahan = request.form['luas_lahan']
        tgl_tanam = request.form['tgl_tanam']
        ...
        eksekusi = cur.execute (query,(luas_lahan, tgl_tanam, tgl_siap_panen, hasil_panen, pemilik, nik, pekerja, id))
        mysql.connection.commit()
        if eksekusi == False:
                return "gagal memasukan data"
        else:
...
@app.route('/padi/hapus/<int:id>', methods=["GET"])
def hapus(id):
    cur = mysql.connection.cursor()
    try:
        eksekusi = cur.execute("DELETE FROM padi where id = %s", (id, ))
        mysql.connection.commit()
        if eksekusi == False:
                return abort(404, "Data tidak ditemukan, mohon di periksa kembali id yang dimasukan",)
        else:
